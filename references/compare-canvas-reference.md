...
| `related`              | 仅用户明确要求时可选，背景节点数组，格式为 `名称     | 说明  | 左侧关系标签 | 右侧关系标签` |
| `force`                | 可选，默认 `false`；不得用 spec 单独请求覆盖 |     |        |         |
| `overwrite_authorized` | 仅用户明确要求覆盖时可写 `true`           |     |        |         |
| `overwrite_reason`     | 仅覆盖时填写，记录用户授权原话或简要理由          |     |        |         |

默认省略 `related`。只有用户明确要求补充相关概念、背景概念或扩展关系时，才允许生成；不得由 agent 自行挑选偏门概念扩张对比范围。即使用户明确要求，也只放直接帮助理解主轴的概念。
...
节点尺寸由脚本根据内容动态估算，不要求 agent 按固定字数或要点数量裁切内容。估算时分别计算中文全角字符、英文与数字、空格和标点的显示宽度；左右详情框分别定高，不为较短一侧强行补齐高度。脚本会比较多组详情框宽度及背景节点网格，选择接近目标宽高比且无重叠的布局，不改变 spec 中的语义内容。
