...
| 调用来源 | 判定规则 |
|---|---|
| `explicit` | 任务以 `/question_note`、`/compare_canvas`、`/open_route` 开头；或明确指定对应入口 |
| `implicit` | 不满足 explicit 规则的一律视为 implicit |

“生成图”“保存”“沉淀”“写入”“生成 canvas”不是显式调用，只是需求信号。

当显式调用为 `/open_route` 或明确要求“进入 open route”时，该 task 的 `request_route` 与 `task_candidate` 都写 `"open_route"`，`has_route_conflict` 写 `false`。`open_route` 表示用户主动绕开标准链路，不因内容像对比或问题沉淀而触发冲突。
...
| 某个目标超出当前标准能力 | 该 task 的 `task_candidate` 写 `"open_route"`，不要发明新 candidate |
...
| `task_queue[].intent.request_route` | 用户显式指定的处理入口 | `"question_note"` / `"compare_canvas"` / `"open_route"` / `null` |
| `task_queue[].intent.task_candidate` | agent 推断的任务候选 | `"question_note"` / `"compare_canvas"` / `"open_route"` |
| `task_queue[].intent.confidence` | 意图解析置信度 | `"high"` / `"medium"` / `"low"` |
| `task_queue[].intent.has_route_conflict` | 显式入口是否和任务内容冲突 | `true` / `false` |
...
| 判定层 | 规则 |
|---|---|
| 无显式入口 | `request_route` 写 `null`，按实际目标填写 `task_candidate`，`has_route_conflict` 写 `false` |
| 显式 `open_route` | `request_route` 与 `task_candidate` 都写 `"open_route"`，`has_route_conflict` 写 `false` |
| 显式标准入口且满足契约 | `task_candidate` 写该入口，`has_route_conflict` 写 `false` |
| 显式标准入口但不满足契约 | `task_candidate` 按实际目标填写，`has_route_conflict` 写 `true` |

当前标准入口契约：

| `request_route` | 可验收产物 | 最小条件 |
|---|---|---|
| `question_note` | 一个问题页 | 有可记录的学习问题；正文可以包含解释、对比或例子 |
| `compare_canvas` | 一个双概念对比 Canvas | 有两个明确可比较概念；用户目标包含对比或要求生成 Canvas |

`confidence` 只表示意图解析是否确定，不等于冲突。比如用户显式要求 `/compare_canvas MCP 是什么`，应写 `confidence = "high"` 且 `has_route_conflict = true`，因为缺少第二个可比较概念。
...
| 边界模糊、复杂组合、或不属于当前标准能力 | `"open_route"` |
